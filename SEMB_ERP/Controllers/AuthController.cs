...
namespace SEMB_ERP.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> Index(string originalPath = "/")
        {
            ...
            List<UserDetailModel> userRole = db.GetUserRole(sesa_id);
            ...
            claimsIdentity.AddClaim(new Claim("semb_erp_name", full_name));
            ...
            if (userDetail.Any() && userRole.Any())
            {
                ...
                if (!string.IsNullOrEmpty(user.level))
                {
                    ...
                    claimsIdentity.AddClaim(new Claim("semb_erp_level", user.level));
                    //claimsIdentity.AddClaim(new Claim("semb_erp_role", user.level));
        ...
        public static void RemoveClaims(ClaimsPrincipal claimsPrincipal, string claimType, string claimValueToRemove)
        {
            ...
            ClaimsIdentity identity = claimsPrincipal.Identities.FirstOrDefault();
            if (identity == null) return;

            // Get the claims to remove
            var claimsToRemove = identity.Claims
                .Where(c => c.Type == claimType && c.Value == claimValueToRemove)
                .ToList();
